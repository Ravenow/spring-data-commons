...
/**
 * {@link MultiValueWithOperationBinding} creates a {@link Predicate} out of given {@link Path} and collection value. Used for
 ...
 */
@FunctionalInterface
public interface MultiValueWithOperationBinding<T extends Path<? extends S>, S> {
	 ...
	 * @param operation the operation type. Will not be {@literal null}.
	 * @return can be {@literal null}, in which case the binding will not be incorporated in the overall {@link Predicate}
	 *         .
	 */
	Predicate bind(T path, Collection<? extends S> value, OperationType operation);
}
